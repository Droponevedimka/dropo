...
Требования: Windows 10/11 x64, WebView2 Runtime, права администратора для
...
```text
app/
  main.go                    # Wails entrypoint, tray, окно
  app_api_*.go               # API для фронтенда (VPN, profiles, fingerprint, …)
  core_*.go                  # storage, routing, WireGuard, filters, telegram, detect
  service_strategies.json    # пер-сервисная база методов desync (v6, go:embed)
  frontend/index.html        # основной UI (инлайн)
dependencies/                # sing-box, wireguard, byedpi, zapret, xray, filters
testlab/                     # лаба-цензор (эмуляция блокировок) — docs/TESTING.md L6
docs/                        # TESTING.md, изображения инструкций
