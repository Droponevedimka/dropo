...
web/TCP/UDP-целях. Режим `Авто` за одну сессию проверяет не более четырёх
кандидатов, после чего фиксирует VPN fallback (либо direct без подписки) до
следующего включения. Сохранённый курсор позволяет следующей сессии продолжить
со следующих четырёх стратегий. Полный контракт режимов `Авто`, `Напрямую`,
`Через VPN` и `Обход (Zapret)` описан в [ROUTING_CONTRACT.md](ROUTING_CONTRACT.md).
