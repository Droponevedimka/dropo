...
func PlatformTargetFor(goos, goarch string) PlatformTarget {
	...
	case "windows":
		return PlatformTarget{
			...
			InstallerAsset:    "dropo-Windows-Setup-" + normalizeReleaseArch(goarch) + ".exe",
			DependenciesAsset: "dropo-Windows-Dependencies-" + normalizeReleaseArch(goarch) + ".zip",
			RequiredDeps:      []string{"sing-box.exe", "winws.exe", "WinDivert.dll"},
			SplitDeps:         true,
			SelfUpdate:        true,
		}
	case "linux":
	...
	case "android":
		return PlatformTarget{
			...
			AppAsset:    "dropo-Android-universal.apk",
			Mobile:      true,
		}
	case "ios":
		return PlatformTarget{
