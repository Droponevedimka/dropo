...
// runGlobalTransparentReselection chooses a single, globally-best transparent
// strategy for the whole VPN session instead of switching the shared winws
// engine per service. winws installs one system-wide WinDivert filter, so only
// one strategy can be active for all traffic at once; per-service switching just
// tears a working engine down for everyone. We sweep each available strategy
// once, score it across all enabled services, then lock the best one.
func (a *App) runGlobalTransparentReselection(reason string) error {
	if a.zapret == nil {
	...
	if !FreeMethodsAllowed(settings) {
		return fmt.Errorf("free methods are disabled")
	}

	strategies := a.zapret.AvailableStrategies()
	if len(strategies) == 0 {
		return fmt.Errorf("no transparent strategies are available")
	}
	scoringServices := make([]FreeAccessService, 0, len(DefaultFreeAccessServices))
	for _, svc := range enabledRouteProbeServices(settings, false) {
		if svc.RequiresVPN || len(svc.ProbeTargets()) == 0 {
			continue
		}
		scoringServices = append(scoringServices, svc)
	}
	if len(scoringServices) == 0 {
		return fmt.Errorf("no services available to score transparent strategies")
	}

	previous := a.zapret.ActiveTag()
	a.writeLog(fmt.Sprintf("[FreeAccess] global transparent reselection started (%s): scoring %d strategy/strategies against %d service(s)",
		reason, len(strategies), len(scoringServices)))

	bestTag, bestLabel, bestWins := "", "", -1
	var bestLatency int64
	for _, strategy := range strategies {
		if a.isShuttingDown() {
			return fmt.Errorf("application is shutting down")
		}
		if err := a.zapret.StartSelected(strategy.Tag); err != nil {
			a.writeLog(fmt.Sprintf("[FreeAccess] reselection: %s failed to start: %v", strategy.Label, err))
			continue
		}
		wins, latency := a.scoreActiveTransparentStrategy(scoringServices)
		a.writeLog(fmt.Sprintf("[FreeAccess] reselection: %s scored %d/%d (avg %d ms)",
			strategy.Label, wins, len(scoringServices), latency))
		if wins > bestWins || (wins == bestWins && wins > 0 && latency < bestLatency) {
			bestTag, bestLabel, bestWins, bestLatency = strategy.Tag, strategy.Label, wins, latency
		}
	}

	target := bestTag
	if target == "" || bestWins <= 0 {
		// Nothing unblocked anything; keep what was active before (or a sane
		// default) rather than leaving the last-probed strategy in place.
		if previous != "" {
			target = previous
		} else if fallback := defaultZapretStrategyTag(a.availableTransparentStrategyTags()); fallback != "" {
			target = fallback
		}
	}
	if target == "" {
		return fmt.Errorf("global transparent reselection found no usable strategy")
	}
	if err := a.zapret.StartSelected(target); err != nil {
		return fmt.Errorf("failed to lock transparent strategy %s: %w", FreeAccessOutboundLabel(target), err)
	}
	if bestTag != "" && bestWins > 0 {
		a.writeLog(fmt.Sprintf("[FreeAccess] global transparent reselection locked %s (%d/%d service(s), avg %d ms)",
			bestLabel, bestWins, len(scoringServices), bestLatency))
	} else {
		a.writeLog(fmt.Sprintf("[FreeAccess] global transparent reselection kept %s (no strategy unblocked any probed service)",
			FreeAccessOutboundLabel(target)))
	}
	a.writeLog(fmt.Sprintf("[NetworkMode] Deep Windows engine active via %s", FreeAccessOutboundLabel(target)))
	return nil
}

// scoreActiveTransparentStrategy probes the services through the engine that is
// already running (no restart) and returns how many fully succeeded plus their
// average latency. Probes run in parallel because they share one engine.
func (a *App) scoreActiveTransparentStrategy(services []FreeAccessService) (int, int64) {
	client := newDirectHTTPClient()
	latencies := make([]int64, len(services))
	var wg sync.WaitGroup
	for i, svc := range services {
		target := strings.TrimSpace(svc.HealthURL)
		if target == "" {
			targets := svc.ProbeTargets()
			if len(targets) == 0 {
				latencies[i] = -1
				continue
			}
		...
		go func(index int, probeURL string) {
			defer wg.Done()
			latency, err := probeHTTPThroughClient(client, probeURL)
			if err != nil {
				latencies[index] = -1
				return
			}
			latencies[index] = latency.Milliseconds()
		}(i, target)
	}
	wg.Wait()
