// Package main provides native WireGuard management for dropo.
...
const (
	// Tunnel naming
	TunnelPrefix       = "dropo-wg-"
	LegacyTunnelPrefix = "kampus-wg-"
...
type NativeWireGuardManager struct {
	...
	onTunnelUnhealthy func(configID int) // Callback before the first restart
}
...
func NewNativeWireGuardManager(basePath string, logger func(string)) *NativeWireGuardManager {
	return NewNativeWireGuardManagerWithData(basePath, basePath, logger)
}

func NewNativeWireGuardManagerWithData(basePath, dataPath string, logger func(string)) *NativeWireGuardManager {
	m := &NativeWireGuardManager{
		basePath:  basePath,
		configDir: filepath.Join(dataPath, "wireguard"),
...
// CleanupOrphanedTunnels removes dropo-wg-* tunnels and legacy kampus-wg-* tunnels.
func (m *NativeWireGuardManager) CleanupOrphanedTunnels() {
	...
	// Query Windows services for dropo and legacy WireGuard tunnel services.
	cmd := exec.Command("sc", "query", "type=", "service", "state=", "all")
	output, err := combinedOutputManagedCommand(cmd, "sc query wireguard services", m.log)
	if err != nil {
		m.log(fmt.Sprintf("Failed to query services: %v", err))
		return
	}

	// Find all dropo-wg-* and legacy kampus-wg-* services.
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(line, "SERVICE_NAME:") {
			...
			if strings.HasPrefix(serviceName, "WireGuardTunnel$"+TunnelPrefix) ||
				strings.HasPrefix(serviceName, "WireGuardTunnel$"+LegacyTunnelPrefix) {
				...
				if stopOutput, stopErr := combinedOutputManagedCommand(stopCmd, "wireguard uninstall orphan tunnel", m.log); stopErr != nil {
...
func (m *NativeWireGuardManager) StartTunnel(configID int, config *WireGuardConfig) error {
	...
	output, err := combinedOutputManagedCommand(cmd, "wireguard install tunnel", m.log)
	if err != nil {
...
func (m *NativeWireGuardManager) StopTunnel(configID int) error {
	...
	output, err := combinedOutputManagedCommand(cmd, "wireguard uninstall tunnel", m.log)
	if err != nil {
...
func (m *NativeWireGuardManager) GetTunnelStats(configID int) (map[string]interface{}, error) {
	...
	output, err := combinedOutputManagedCommand(cmd, "wg show", m.log)
	if err != nil {
...
// SetTunnelUnhealthyCallback sets the fail-safe callback invoked outside the
// manager lock before an unhealthy tunnel is restarted.
func (m *NativeWireGuardManager) SetTunnelUnhealthyCallback(callback func(configID int)) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.onTunnelUnhealthy = callback
}
...
func (m *NativeWireGuardManager) checkAllTunnels() {
	...
	for _, state := range tunnelsToCheck {
		healthy, lastHandshake, healthErr := m.checkTunnelHealth(state.ConfigID)
		expectsPeriodicHandshake := wireGuardConfigExpectsPeriodicHandshake(state.Config)
		// A peer without PersistentKeepalive is allowed to be completely idle;
		// WireGuard does not handshake until traffic is sent. Treating "never"
		// or a stale handshake as failure caused needless 30-second restart loops.
		if healthErr == nil && !expectsPeriodicHandshake {
			healthy = true
		} else if healthErr == nil && lastHandshake.IsZero() && time.Since(state.StartedAt) < HandshakeTimeout {
			// Give keepalive-enabled peers a full handshake window after start.
			healthy = true
		}
		rollbackCamouflage := healthErr == nil && expectsPeriodicHandshake && !healthy
		...
		if tunnelState, exists := m.tunnels[state.Name]; exists {
			...
			if !healthy && tunnelState.RestartCount < MaxRestartAttempts && tunnelState.Config != nil {
				...
				restartCount := tunnelState.RestartCount
				configID := tunnelState.ConfigID
				config := tunnelState.Config
				onUnhealthy := m.onTunnelUnhealthy
				onRestart := m.onTunnelRestart
				m.mu.Unlock()

				m.log(fmt.Sprintf("Attempting to restart tunnel %s (attempt %d/%d)",
					state.Name, restartCount, MaxRestartAttempts))
				if restartCount == 1 && rollbackCamouflage && onUnhealthy != nil {
					onUnhealthy(configID)
				}

				if err := m.restartTunnel(configID, config); err != nil {
...
func wireGuardConfigExpectsPeriodicHandshake(config *WireGuardConfig) bool {
	if config == nil {
		return false
	}
	for _, peer := range config.Peers {
...
func (m *NativeWireGuardManager) checkTunnelHealth(configID int) (bool, time.Time, error) {
	...
	return healthy, lastHandshake, nil
}
