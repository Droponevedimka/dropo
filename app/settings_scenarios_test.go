...
func TestDefaultStorageSettingsMatchCurrentNetworkPolicy(t *testing.T) {
	...
	if !settings.EnableLogging || settings.LogLevel != LogLevelTrace {
		t.Fatalf("logging defaults = enabled:%t level:%q, want enabled trace", settings.EnableLogging, settings.LogLevel)
	}
