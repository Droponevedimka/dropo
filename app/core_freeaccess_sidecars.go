...
type ProxyBypassManager struct {
	basePath string
	methods  []ProxyFreeAccessMethod
	logger   func(string)
...
func NewProxyBypassManager(basePath string, methods []ProxyFreeAccessMethod, logger func(string)) *ProxyBypassManager {
	return &ProxyBypassManager{
		basePath: basePath,
		methods:  methods,
		logger:   logger,
...
func (m *ProxyBypassManager) log(message string) {
	if m.logger != nil {
		m.logger(fmt.Sprintf("[FreeAccess] %s", message))
	}
}

func (m *ProxyBypassManager) methodPath(method ProxyFreeAccessMethod) string {
	return filepath.Join(m.basePath, "bin", method.ExeName)
}

func (m *ProxyBypassManager) IsInstalled() bool {
	for _, method := range m.methods {
		if !methodSupportsCurrentPlatform(method.Platforms) {
			continue
		}
		if fileExists(m.methodPath(method)) {
			return true
		}
	}
	return false
...
func (m *ProxyBypassManager) Start() error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	for _, method := range m.methods {
		...
		if _, running := m.cmds[method.Tag]; running {
			started++
			continue
		}
		if err := m.startMethodLocked(method); err != nil {
			lastErr = err
			m.log(fmt.Sprintf("%s failed to start: %v", method.Label, err))
			continue
		}
		started++
	}
	...
	if started == 0 {
		...
		return fmt.Errorf("no proxy free-access method is available")
	}
	return nil
}

func (m *ProxyBypassManager) startMethodLocked(method ProxyFreeAccessMethod) error {
	exePath := m.methodPath(method)
	if !fileExists(exePath) {
		return fmt.Errorf("%s not found: %s", method.ExeName, exePath)
	}

	cmd := exec.Command(exePath, method.Args...)
	...
	attachManagedCmdToJob(cmd, method.Label, m.log)
	if stdoutErr == nil {
		go streamCmdOutput(stdout, method.Label+" OUT", m.log)
	}
	if stderrErr == nil {
		go streamCmdOutput(stderr, method.Label+" ERR", m.log)
	}

	m.cmds[method.Tag] = cmd
	m.log(fmt.Sprintf("%s started on 127.0.0.1:%d (pid=%d)", method.Label, method.Port, cmd.Process.Pid))
	go m.waitForMethodExit(method, cmd)
	return nil
}
...
func (m *ProxyBypassManager) waitForMethodExit(method ProxyFreeAccessMethod, cmd *exec.Cmd) {
	err := cmd.Wait()
	m.mu.Lock()
	if m.cmds != nil && m.cmds[method.Tag] == cmd {
		delete(m.cmds, method.Tag)
	}
	m.mu.Unlock()

	if err != nil {
		m.log(fmt.Sprintf("%s exited: %v", method.Label, err))
		return
	}
	m.log(fmt.Sprintf("%s exited", method.Label))
}

func (m *ProxyBypassManager) ActiveTags() []string {
	m.mu.Lock()
	running := make(map[string]bool, len(m.cmds))
	for tag := range m.cmds {
		running[tag] = true
	}
	methods := append([]ProxyFreeAccessMethod(nil), m.methods...)
	m.mu.Unlock()

	tags := make([]string, 0, len(methods))
	for _, method := range methods {
		if running[method.Tag] && loopbackPortReady(method.Port, 300*time.Millisecond) {
...
func (m *ProxyBypassManager) WaitForActiveTags(timeout time.Duration) []string {
...
func (m *ProxyBypassManager) Stop() {
	m.mu.Lock()
