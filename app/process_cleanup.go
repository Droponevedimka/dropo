...
func (a *App) cleanupManagedSidecarOrphansAsync(reason string) {
	if a == nil || runtime.GOOS != "windows" || a.basePath == "" {
		return
	}
	go a.cleanupManagedSidecarOrphans(reason)
}
...
func (a *App) managedSidecarPaths() []string {
	...
	candidates := []string{
		a.singboxPath,
		filepath.Join(basePath, "bin", ByeDPIProcessName),
		filepath.Join(basePath, "bin", SpoofDPIExeName),
		filepath.Join(basePath, "bin", ZapretProcessName),
		filepath.Join(basePath, "bin", XrayExeName),
		filepath.Join(basePath, "bin", TgWsProxyProcessName),
	}
...
func waitForWindowsPIDExit(pid int, timeout time.Duration) {
	...
	script := fmt.Sprintf("Wait-Process -Id %d -Timeout %d", pid, seconds)
	_ = newBackgroundCommand("powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", script).Run()
}
