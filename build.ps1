...
function Build-Application {
    ...
    if (Test-Path $ReleaseDir) {
        $oldBuilds = Get-ChildItem -Path $ReleaseDir -Directory | Where-Object { $_.Name -match "^(dropo-)?$([regex]::Escape($AppVersion))-[0-9a-f]+$" }
        ...
        # Also remove old ZIP files for the same version.
        $oldZips = Get-ChildItem -Path $ReleaseDir -File -Filter "dropo-$AppVersion*.zip"
    ...
    # changes; keyed by depsVersion so releases can reuse it). The platform tag
    # keeps room for future dropo-<platform>-* assets (Android/macOS/Linux/iOS).
    $DepsAsset = "dropo-Windows-Dependencies-x64-$DepsVersion.zip"
    ...
    # and host this depsVersion's archive in a release once (the client bootstrap
    # finds it by name across releases).
    $lock = [ordered]@{ depsVersion = $DepsVersion; asset = $DepsAsset; sha256 = $depsZipSha; size = $depsZipSize }
    [System.IO.File]::WriteAllText((Join-Path $ScriptRoot "deps-lock.json"), ($lock | ConvertTo-Json), (New-Object System.Text.UTF8Encoding($false)))
    ...
    [System.IO.File]::WriteAllText((Join-Path $AppFolder "dependencies.json"), ($manifest | ConvertTo-Json), (New-Object System.Text.UTF8Encoding($false)))
    Write-Host "[OK] Wrote dependencies.json (depsVersion=$DepsVersion)" -ForegroundColor Green
